...
public class kuis1 {
	public static void main(String[] args) {
		...
		int lama = 0;
		//kondisi
		if (masuk < keluar && masuk <= 24 && keluar <= 24 && masuk != keluar) {
		  // Format 24 jam
		  lama += keluar - masuk;
		} else if (masuk > keluar || masuk == keluar && masuk <= 12 && keluar <= 12) {
		  // Format 12 jam
		  lama += (keluar + 12) - masuk;
		} else {
		  // error output
		  System.out.println(ANSI_RED + "\n\n\nError" + ANSI_RESET + ": Nilai input melebihi format waktu atau input jam sama");
		  System.exit(0);
		}
